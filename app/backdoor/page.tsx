...
export default function Backdoor() {
    ...
    const handleAuth = async (e: React.FormEvent) => {
        e.preventDefault();
        setAuthError(false);
        // Hardcoded check since .env is gone
        if (passwordInput === "bandana-king") {
            setIsAuthenticated(true);
    ...
    const handleCommand = async (e: React.FormEvent) => {
        ...
        if (cmd === "help") {
            ...
            addLog("  status            - CHECK STREET OPS");
        } else if (cmd === "logout") {
            ...
            addLog("STREET_CODE_CHRONICLES V.3.0");
            addLog("KERNEL: STATIC_HARDCODED_STABLE");
            addLog("UI: TAILWIND_V4_NEO_ASPHALT");
        } else if (cmd === "status") {
            addLog("DATABASE: DECOMMISSIONED");
            addLog("LEDGER: HARDCODED_STASH");
            addLog("STREET_CRED: MAXIMUM");
        } else {
            ...
            <div className="max-w-4xl mx-auto border-2 border-neon-green/30 bg-asphalt/50 backdrop-blur-md shadow-[0_0_20px_rgba(10,255,0,0.1)] rounded-sm flex flex-col h-[80vh]">
                ...
                <div className="border-b border-neon-green/30 p-3 flex items-center justify-between bg-concrete/20">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold tracking-widest text-white">THE_BACKDOOR_TERMINAL_V.3.0</span>
                    </div>
